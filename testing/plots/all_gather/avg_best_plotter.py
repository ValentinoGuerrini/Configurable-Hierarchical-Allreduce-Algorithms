...
if len(sys.argv) < 2:
    print("Usage: python avg_best_plotter.py <filename>")
    sys.exit(1)
...
avg = (
    df.groupby(['nprocs', 'send_count', 'algorithm'])['time']
      .mean()                 # <-- average instead of median
      .reset_index()
)

# To wide: index=(nprocs, send_count), columns=algorithm, values=avg time
wide = avg.pivot(index=['nprocs','send_count'], columns='algorithm', values='time')
...
# ---------------- Winners & speedup (using averages) ----------------
# Global best (lowest AVERAGE time) per (nprocs, send_count)
...
# Best-of-mine vs baseline (exclude baseline when picking "mine"), using AVERAGES
...
algos = sorted(avg['algorithm'].unique())
...
ax.set_title('Best Allgather Algorithm by nprocs & send_count (AVERAGE time)')
